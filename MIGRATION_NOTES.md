...
## Authorization cache and Valkey/Redis snapshot

This template uses Valkey/Redis as the runtime permission snapshot cache.

- Login and refresh publish an effective permission snapshot.
- Request authentication re-hydrates permissions from Valkey/Redis and only falls back to the database when the cached authority version is stale or missing.
