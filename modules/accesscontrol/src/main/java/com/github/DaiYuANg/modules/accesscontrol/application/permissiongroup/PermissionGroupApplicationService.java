...
public class PermissionGroupApplicationService {
    ...
    public PermissionGroupVO updatePermissionGroup(Long id, UpdatePermissionGroupForm form) {
        ...
        auditSupport.bumpGlobalVersion();
        auditSupport.record("permission-group", "update", String.valueOf(id), true, "update permission group");
        return toPermissionGroupVOWithCatalog(group);
    }

    @Transactional
    public void deletePermissionGroup(Long id) { authorizationService.check("permission-group", "delete"); repository.deleteById(id); auditSupport.bumpGlobalVersion(); auditSupport.record("permission-group", "delete", String.valueOf(id), true, "delete permission group"); }
    public PageResult<PermissionGroupVO> queryPermissionGroupPage(PermissionGroupQuery query) {
        authorizationService.check("permission-group", "view");
        var slice = repository.page(query);
        return PageResult.of(slice.total(), query.getPageNum(), query.getPageSize(), slice.content().stream().map(this::toPermissionGroupVO).toList());
    }
    ...
    public void assignPermissions(PermissionGroupRefPermissionForm form) {
        ...
        auditSupport.bumpGlobalVersion();
        auditSupport.record("permission-group", "assign-permission", String.valueOf(form.permissionGroupId()), true, "assign permissions");
    }
    ...
    private PermissionGroupVO toPermissionGroupVO(com.github.DaiYuANg.accesscontrol.projection.PermissionGroupListProjection group) {
