...
public class UserApplicationService {
    ...
    public Optional<UserVO> getUserById(Long id) { authorizationService.check("user", "view"); return userRepository.findByIdOptional(id).map(this::toUserVO); }
    public List<UserVO> getAllUsers() { authorizationService.check("user", "view"); return userRepository.listAll().stream().map(this::toUserVO).toList(); }
    ...
    public Optional<UserVO> getUserByUsername(String username) { authorizationService.check("user", "view"); return userRepository.findByUsername(username).map(this::toUserVO); }
    ...
    public void assignRole(UserRefRoleForm form) {
        ...
        auditSupport.bumpGlobalVersion();
        auditSupport.record("user", "assign-role", String.valueOf(form.userId()), true, "assign user roles");
    }
    ...
    private UserVO toUserVO(SysUser user) {
        var roles = user.roles == null
            ...
            : user.roles.stream().map(this::toRoleVO).collect(Collectors.toCollection(LinkedHashSet::new));
        return new UserVO(
            user.id,
            user.username,
            user.identifier,
    ...
    private RoleVO toRoleVO(com.github.DaiYuANg.accesscontrol.entity.SysRole role) {
        var groups = role.permissionGroups == null
            ? new LinkedHashSet<PermissionGroupVO>()
            : role.permissionGroups.stream().map(this::toPermissionGroupVO).collect(Collectors.toCollection(LinkedHashSet::new));
        return new RoleVO(role.id, role.name, role.code, role.status, role.sort, role.createAt, role.updateAt, groups);
    }

    private PermissionGroupVO toPermissionGroupVO(com.github.DaiYuANg.accesscontrol.entity.SysPermissionGroup group) {
        var permissions = group.permissions == null
            ? new LinkedHashSet<PermissionVO>()
            : group.permissions.stream().map(this::toPermissionVO).collect(Collectors.toCollection(LinkedHashSet::new));
        return new PermissionGroupVO(group.id, group.name, group.description, group.code, group.sort, group.createAt, group.updateAt, permissions);
    }

    private PermissionVO toPermissionVO(com.github.DaiYuANg.accesscontrol.entity.SysPermission permission) {
        return new PermissionVO(permission.id, permission.name, permission.code, permission.resource, permission.action, permission.groupCode, permission.description, permission.expression);
    }
